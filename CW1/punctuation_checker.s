...
isCharorEoF:  #check if $a0 is char, argument: $a0(char), variables:  $a1(compare char),$a3(return value
    beq $a0, $zero, true     #check end of line char
    addi $a1, $zero, 65
...
markedprintloop: 
    ...
    #addi $t1,$t1, 2
    bgt $s0, $t1,  endofprint   #end printing if $s0 would be bigger than tokens_number
    lb $t0, marks($s0)
...
CHARLOOP:
    ...
    beq $zero, $a3, PunOrSp  #if not char,or '\0'($a3=0) we are not interested, so return
    lb, $s2, dictionary($s0)   
...
END:
    ...
    j markloop
#nothing gets here except direct call, and seemed easier to write it here, instead of calling it as a function
#checks punctuations, if incorrect by the rules we mark them
#always starts at punctuation token's first character, as it is already not a char or'/0' 
PunOrSp:
    li $t1, 32
    beq $t1, $s3, END         #if space, return, we dont mark spaces
    lw $t0, tokens_number     #load tokens_number for later use
check1:
    addi $t2, $s5, -1         # the token's number before the token we are checking
    blt $t2, $zero, check2    #if %s5-1<0 than we are at first token so we dont need to check if it has a space before it
    addi $t3, $s6,-201        # take away 201 from the full address of current token to get the address of the one before
    lb $t4, ($t3)             #load first char of that token
    beq $t4, $t1, incorrect   #if that char is a space, punct. is incorrect
check2:
    addi $t2, $s5, 1          # the token's number after the token we are checking
    bgt $t2, $t0, check3      #if %s5+1>tokens_number than we are at last token so we dont need to check if it has a char after it
    addi $t3, $s6, 201        # add 201 to the full address of current token to get the address of the next one
    lb $t4, ($t3)             # load first char of that token
    addi $a1, $zero, 65
    blt $t4, $a1, check3      #notchar as c < 'A' so its not a char
    addi $a1, $zero, 90
    ble $t4, $a1, incorrect   #incorrect as c >= 'A' && c <= 'Z' so it has a char after it
    addi $a1, $zero, 97
    blt $t4, $a1, check3      #notchar as c > 'Z' && c < 'a' so its not a char after the token
    addi $a1, $zero, 122
    ble $t4, $a1, incorrect   #incorrect as c >= 'a' && c <= 'z' so it has a char after it
check3:
    li $t2, 46                #load dot ascii 
    bne $s3, $t2, check4      #if not a dot, we go on to next check
    addi $t3, $s6, 1          #same token's 2. char
    lb $t4, ($t3)             #load char
    bne $t4, $t2, check4      #if not a dot, we go on to next check (includes '\0' so if token ended we dont go any further)
    addi $t3, $s6, 2          #same token's 3. char
    lb $t4, ($t3)             #load char
    bne $t4, $t2, check4      #if not a dot, we go on to next check
    addi $t3, $s6, 3          #same token's 4. char
    lb $t4, ($t3)             #load char
    beq $t4, $zero, correct   #if not a '\0', we go on to next check, otherwise correct                
     # if all 3 if passed, we have 3 dots,  but not sure if only 3 so we check for 4. char to be '\0'   
check4:
    addi $t3, $s6, 1          #token's 2. char
    lb $t4, ($t3)             #load char
    beq $t4, $zero, correct   #if not a '\0', it is incorrect from now on, but for formalities I go on...
check5:
    addi $t3, $s6, 1          #token's 2. char
    lb $t4, ($t3)             #load char
    bne $t4, $zero, incorrect #token only contains punctuations, so anyhing but a '\0' here is incorrect
check6:
    j incorrect               #all other variations are incorrect
correct:
    j END
incorrect:
    li $a0, 1
    sb $a0, marks($s5)       #marks punctuation
    j END
S_C_END:
